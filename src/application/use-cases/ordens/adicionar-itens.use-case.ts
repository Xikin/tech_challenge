...
export class AdicionarItensUseCase {
  ...
  async execute(id: string, data: AdicionarItensInput) {
    ...
    if (!["RECEBIDA", "EM_DIAGNOSTICO", "AGUARDANDO_APROVACAO"].includes(os.status as string))
    ...
    const pecas = await Promise.all(
      data.pecas.map(async ({ pecaId, quantidade }) => {
        ...
        if (p.quantidade < quantidade)
          throw new StockError(`Estoque insuficiente para "${p.nome}". Disponível: ${p.quantidade}`);
        return { peca: p, quantidade };
      }),
    );

    const addServicos = servicos.reduce((acc, s) => acc + Number(s.preco), 0);
    const addPecas = pecas.reduce((acc, { peca, quantidade }) => acc + Number(peca.preco) * quantidade, 0);
    ...
    await this.repo.adicionarItens({
      ...
      pecas: pecas.map(({ peca, quantidade }) => ({ pecaId: peca.id, quantidade, preco: Number(peca.preco) })),
      novoTotal,
    });
