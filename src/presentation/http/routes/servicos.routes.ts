...
import {
  criarServicoSchema, atualizarServicoSchema, listarServicosSchema, paramsIdSchema,
  erroResponseSchema, tempoMedioResponseSchema, servicoResponseSchema, listarServicosResponseSchema,
...
export const servicosRoutes: FastifyPluginAsync = async (instance) => {
  ...
  fastify.post("/", { onRequest: [autenticar], schema: { tags, security, summary: "Criar serviço", body: criarServicoSchema, response: { 201: servicoResponseSchema, 401: erroResponseSchema, 409: erroResponseSchema } } },
    async (req, rep) => rep.status(201).send(await criar.execute(req.body)));

  fastify.get("/", { onRequest: [autenticar], schema: { tags, security, summary: "Listar serviços", querystring: listarServicosSchema, response: { 200: listarServicosResponseSchema } } },
    async (req, rep) => rep.send(await listar.execute(req.query)));

  fastify.get("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Buscar serviço por ID", params: paramsIdSchema, response: { 200: servicoResponseSchema, 404: erroResponseSchema } } },
    async (req, rep) => rep.send(await buscarPorId.execute(req.params.id)));

  fastify.get("/:id/tempo-medio", { onRequest: [autenticar], schema: { tags, security, summary: "Tempo médio de execução", params: paramsIdSchema, response: { 200: tempoMedioResponseSchema, 404: erroResponseSchema } } },
    async (req, rep) => rep.send(await calcularTempoMedio.execute(req.params.id)));

  fastify.put("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Atualizar serviço", params: paramsIdSchema, body: atualizarServicoSchema, response: { 200: servicoResponseSchema, 404: erroResponseSchema, 409: erroResponseSchema } } },
    async (req, rep) => rep.send(await atualizar.execute(req.params.id, req.body)));

  fastify.delete("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Remover serviço", params: paramsIdSchema, response: { 404: erroResponseSchema, 400: erroResponseSchema } } },
