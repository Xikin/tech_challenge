...
import {
  criarVeiculoSchema, atualizarVeiculoSchema, listarVeiculosQuerySchema,
  paramsIdSchema, placaParamsSchema, veiculoResponseSchema, erroResponseSchema,
...
export const veiculosRoutes: FastifyPluginAsync = async (instance) => {
  ...
  fastify.post("/", { onRequest: [autenticar], schema: { tags, security, summary: "Cadastrar veículo", body: criarVeiculoSchema, response: { 201: veiculoResponseSchema, 404: erroResponseSchema, 409: erroResponseSchema, 422: erroResponseSchema } } },
    async (req, rep) => rep.status(201).send(await criar.execute(req.body as CriarVeiculoData)));

  fastify.get("/", { onRequest: [autenticar], schema: { tags, security, summary: "Listar veículos", querystring: listarVeiculosQuerySchema, response: { 200: z.array(veiculoResponseSchema) } } },
    async (req, rep) => rep.send(await listar.execute(req.query.clienteId)));

  fastify.get("/placa/:placa", { onRequest: [autenticar], schema: { tags, security, summary: "Buscar por placa", params: placaParamsSchema, response: { 200: veiculoResponseSchema, 404: erroResponseSchema } } },
    async (req, rep) => rep.send(await buscarPorPlaca.execute(req.params.placa)));

  fastify.get("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Buscar por ID", params: paramsIdSchema, response: { 200: veiculoResponseSchema, 404: erroResponseSchema } } },
    async (req, rep) => rep.send(await buscarPorId.execute(req.params.id)));

  fastify.put("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Atualizar veículo", params: paramsIdSchema, body: atualizarVeiculoSchema, response: { 200: veiculoResponseSchema, 404: erroResponseSchema } } },
    async (req, rep) => rep.send(await atualizar.execute(req.params.id, req.body)));

  fastify.delete("/:id", { onRequest: [autenticar], schema: { tags, security, summary: "Remover veículo", params: paramsIdSchema, response: { 404: erroResponseSchema } } },
