...
const makeRepo = () =>
  ({
    ...
    buscarCliente: vi.fn(),
    buscarVeiculo: vi.fn(),
  }) as unknown as IOrdemRepository;

const makeEmailService = () =>
  ({
    enviarStatusAtualizado: vi.fn().mockResolvedValue(undefined),
...
describe("AvancarStatusUseCase — máquina de estados", () => {
  ...
  fluxo.forEach(([de, para]) => {
    it(`${de} → ${para}`, async () => {
      ...
      await new AvancarStatusUseCase(repo, emailService).execute(OS_ID, {});
...
describe("CriarOrdemUseCase", () => {
  it("lança NotFoundError cliente inexistente", async () => {
    ...
    await expect(
      new CriarOrdemUseCase(repo).execute({
        clienteId: CLI_ID,
        veiculoId: VEI_ID,
